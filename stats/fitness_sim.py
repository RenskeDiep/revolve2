...
for exp in sim_065_1:  # loop over experiments
    for gen in sim_065_1[exp]:  # loop over generations
        if gen == last_gen:  # only look at the last generation
            sim_scores = sim_065_1[exp][gen].get("sim_scores", [])
            avg_sim= np.mean(sim_scores)
            fitness_scores = fitness_065_1[exp][gen]
            avg_fitness = np.mean(fitness_scores)
            sim_fit_065_1[exp] = (avg_sim, avg_fitness)

for exp in sim_stand:  # loop over experiments
    for gen in sim_stand[exp]:  # loop over generations
        if gen == last_gen:  # only look at the last generation
            sim_scores = sim_stand[exp][gen].get("sim_scores", [])
            avg_sim= np.mean(sim_scores)
            fitness_scores = fitness_stand[exp][gen]
            avg_fitness = np.mean(fitness_scores)
            sim_fit_stand[exp] = (avg_sim, avg_fitness)
...
# Unpack data for third condition (0.65–1)
sims_065_1 = [v[0] for v in sim_fit_065_1.values()]
fitness_065_1 = [v[1] for v in sim_fit_065_1.values()]

# Unpack data for fourth condition (standard)
sims_stand = [v[0] for v in sim_fit_stand.values()]
fitness_stand = [v[1] for v in sim_fit_stand.values()]
